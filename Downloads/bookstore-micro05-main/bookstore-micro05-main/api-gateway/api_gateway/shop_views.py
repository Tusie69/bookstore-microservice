...
def shop_checkout(request):
    ...
    if request.method == "POST":
        form_token = request.POST.get("checkout_token", "")
        session_token = request.session.get("checkout_token", "")
        if not form_token or form_token != session_token:
            messages.warning(request, "This checkout request was already processed. Please review your orders.")
        ...
        cart_item_ids = request.POST.getlist("item_id")
        ...
        if r and r.status_code == 201:
            for item_id in item_ids:
                try:
                    http_requests.delete(f"{CART_SERVICE_URL}/cart-items/{item_id}/", timeout=5)
                except Exception:
                    pass
            messages.success(request, "Order placed successfully! Payment and shipment have been initiated.")
            return redirect("shop_orders")
        else:
            request.session["checkout_token"] = str(uuid4())
